...
class FtiqCrmApiBase(http.Controller):
    ...
    def _invoice_metrics_by_partner(self, partners):
        ...
        for move in moves:
            ...
            due_date = self._field_value(move, "invoice_date_due")
            if due_date and due_date < today:
                values["overdue_amount"] += self._field_value(move, "amount_residual", 0.0) or 0.0
                values["overdue_amount_company"] += abs(
                    self._field_value(move, "amount_residual_signed", 0.0) or 0.0
                )
                values["overdue_invoice_count"] += 1
        for values in metrics.values():
            ...
            if len(currencies) == 1:
                ...
                values["overdue_amount"] = values["overdue_amount_company"]
            values.pop("due_amount_company", None)
            values.pop("overdue_amount_company", None)
        return metrics
    ...
    def _serialize_mail_notification(self, notification):
        ...
        mobile_notification_type = payload["data"].get("notification_type", "")
        notification_type = "system"
        if mobile_notification_type == "task_chatter":
            notification_type = "taskChatter"
        elif related_model == "project.task":
    ...
    def _serialize_task(self, task):
        ...
        parent_task = self._field_value(task, "parent_id")
        subtasks, subtask_count = self._serialize_task_subtasks(task)
        execution_payload = {}
        ...
        return {
            ...
            "parent_id": str(parent_task.id) if parent_task else "",
            "parent_title": self._field_value(parent_task, "name", "") if parent_task else "",
    ...
    def _serialize_task_subtasks(self, task):
        if not task or "child_ids" not in task._fields:
            return [], 0
        try:
            child_tasks = self._field_value(task, "child_ids", request.env["project.task"])
        except AccessError:
            return [], 0
        result = []
        for child in child_tasks[:10]:
            state_value = self._task_state_code(child)
            result.append(
                {
                    "id": str(child.id),
                    "title": self._field_value(child, "name", "") or "",
                    "status": self._task_status_value(child, state_value=state_value),
                    "status_code": state_value,
                    "priority": self._task_priority_value(child),
                    "due_date": self._display_date(child, "date_deadline"),
                }
            )
        return result, len(child_tasks)
    ...
    def _serialize_messages(self, record):
        ...
        for message in messages:
            ...
            subtype = self._field_value(message, "subtype_id")
            subtype_xmlid = ""
            if subtype:
                subtype_xmlid = subtype.get_external_id().get(subtype.id, "")
            message_kind = "note" if subtype_xmlid == "mail.mt_note" else "message"
            body = html2plaintext(self._field_value(message, "body", "") or "").strip()
            result.append(
                {
                    "id": str(message.id),
                    "comment": body,
                    ...
                    "commented_by": {
                        ...
                        "name": self._record_name(author, "") if author else "",
                        "user_details": {
                            "email": user_email,
                            "name": self._record_name(author, "") if author else "",
                            "profile_pic": "",
