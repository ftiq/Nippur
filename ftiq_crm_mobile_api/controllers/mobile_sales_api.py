...
class FtiqCrmMobileSalesApi(FtiqCrmApiBase):
    ...
    def _client_sale_order_create(self, partner_id):
        ...
        sales_task = self._mobile_task_from_payload(payload, partner, "sales_order")
        line_values = self._validated_order_lines(payload)
        order_values = self._sale_order_create_values(partner, payload.get("note"))
        if sales_task:
            if "task_id" in SaleOrder._fields:
                order_values["task_id"] = sales_task.id
            elif "origin" in SaleOrder._fields:
                order_values["origin"] = sales_task.display_name or sales_task.name or ""
        ...
        if sales_task:
            sales_task.message_post(
                body=Markup(
                    "<p>%s</p>"
                    % escape(
                        _("Sales order created from the mobile application: %s")
                        % (order.display_name or order.name or order.id)
                    )
                )
            )
            self._mark_mobile_task_done(sales_task)
        return self._json(
            {
