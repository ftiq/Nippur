...
class FtiqCrmMobileApi(FtiqCrmApiBase):
    ...
    def _tasks(self):
        if request.httprequest.method == "POST":
            ...
            task = self._mobile_task_model(
                ...
                default_parent_id=task_values.get("parent_id"),
            ).create(task_values)
    ...
    def _task_execution(self, task_id):
        ...
        if not task:
            return self._error(_("Task not found."), status=404)
        flow_error = self._task_execution_flow_error(task)
        if flow_error:
            return flow_error
    ...
    def _task_visit_complete(self, task_id):
        ...
        done_stage = self._task_stage_from_status(task, "done")
        if done_stage:
            values["stage_id"] = done_stage.id
        if "fsm_done" in task._fields:
    ...
    def _task_visit_cancel(self, task_id):
        ...
        if state:
            values["state"] = state
        cancelled_stage = self._task_stage_from_status(task, "cancelled")
        if cancelled_stage:
    ...
    def _task_execution_flow_error(self, task):
        task_type = self._field_value(task, "ftiq_mobile_task_type", "") or "field_visit"
        if task_type in {"field_visit", "stock_audit"}:
            return None
        return self._error(
            _("This task type uses a dedicated workflow and cannot be opened as a field visit."),
            status=400,
        )
    ...
    def _task_visit_message_body(self, task, title, payload=None, report=None):
        ...
        for product in products:
            ...
            product_rows += (
                ...
                "<div style='font-weight:400;color:#111827;font-size:10px;margin-top:2px;'>%s</div></td>"
        ...
        for line in stock_lines:
            ...
            competitor_details = "%s" % escape(line.get("competitor_product") or "-")
            if line.get("competitor_qty") not in (None, False, ""):
                competitor_details += (
                    "<div style='color:#111827;font-size:10px;margin-top:2px;'>%s: %s</div>"
            ...
            stock_line_rows += (
                "<tr style='background:#ffffff;'>"
                "<td style='padding:8px;border-bottom:1px solid #e5e7eb;width:62px;'>%s</td>"
                "<td style='padding:8px;border-bottom:1px solid #e5e7eb;'>%s</td>"
                "<td style='padding:8px;border-bottom:1px solid #e5e7eb;color:#111827;font-weight:700;text-align:center;'>%s</td>"
                "<td style='padding:8px;border-bottom:1px solid #e5e7eb;color:#111827;'>%s</td>"
                "<td style='padding:8px;border-bottom:1px solid #e5e7eb;color:#111827;'>%s</td>"
                "</tr>"
            ) % (
                shelf_photo,
        ...
        if stock_line_rows:
            stock_lines_table = (
                "<div class='ftiq-mobile-note__section ftiq-mobile-note__section--info' style='margin-top:12px;border:1px solid #bfdbfe;border-radius:10px;overflow:hidden;background:#eff6ff;'>"
                "<div class='ftiq-mobile-note__title' style='padding:8px 10px;background:#dbeafe;color:#111827;font-weight:800;font-size:12px;'>%s</div>"
                "<table class='ftiq-mobile-note__table' style='width:100%%;border-collapse:collapse;background:white;font-size:11px;'>"
                "<thead><tr style='background:#dbeafe;color:#111827;'>"
                "<th style='padding:7px 8px;text-align:right;width:62px;'>%s</th>"
                "<th style='padding:7px 8px;text-align:right;'>%s</th>"
                "<th style='padding:7px 8px;text-align:center;'>%s</th>"
                "<th style='padding:7px 8px;text-align:right;'>%s</th>"
                "<th style='padding:7px 8px;text-align:right;'>%s</th>"
                "</tr></thead><tbody>%s</tbody></table></div>"
            ) % (
        ...
        return Markup(
            "<div class='ftiq-mobile-note' style='max-width:760px;border:1px solid #d8dee4;border-radius:12px;overflow:hidden;"
            "background:#ffffff;direction:rtl;text-align:right;font-size:11px;line-height:1.45;'>"
            "<div class='ftiq-mobile-note__header' style='padding:10px 12px;background:#f8fafc;color:#111827;border-bottom:1px solid #e5e7eb;'>"
            "<div class='ftiq-mobile-note__title' style='font-size:13px;font-weight:900;'>%s</div>"
            "<div class='ftiq-mobile-note__subtitle' style='margin-top:3px;color:#111827;font-size:11px;'>%s</div></div>"
    ...
    def _task_values(self, payload, existing=False, partial=False):
        Task = self._mobile_task_model()
        parent_task = self._task_parent_from_payload(payload) if not existing else False
        values = {}
        if not partial or "title" in payload:
        ...
        if not partial or "assigned_to" in payload:
            ...
            if parent_task and "assigned_to" not in payload:
        ...
        if "ftiq_mobile_task_type" in Task._fields and (
            ...
            task_type = (
                ...
                or self._field_value(parent_task, "ftiq_mobile_task_type")
                or ("" if existing or parent_task else "field_visit")
            )
        ...
        if partner:
            ...
            parent_partner = self._field_value(parent_task, "partner_id")
            if parent_partner:
                values["partner_id"] = parent_partner.id
        if "project_id" in Task._fields:
            ...
            if payload.get("project_id") not in (None, False, ""):
                ...
                project = candidate
            if not project and parent_task:
                project = self._field_value(parent_task, "project_id")
            if not project and (not existing or not self._field_value(existing, "project_id")):
                project = self._default_project()
            if project:
                values["project_id"] = project.id
        if parent_task and "company_id" in Task._fields:
            parent_company = self._field_value(parent_task, "company_id")
            if parent_company:
                values["company_id"] = parent_company.id
        if parent_task and "parent_id" in Task._fields:
            values["parent_id"] = parent_task.id
        return values

    def _task_parent_from_payload(self, payload):
        parent_value = payload.get("parent_id") or payload.get("parent_task_id")
        if parent_value in (None, False, ""):
            return self._mobile_task_model()
        try:
            parent_id = int(parent_value)
        except Exception:
            raise UserError(_("Invalid parent task."))
        parent_task = self._task_record_for_mobile(parent_id)
        if not parent_task:
            raise UserError(_("Parent task is not available for this user."))
        return parent_task
    ...
    def _task_stage_from_status(self, task, status):
        project = self._field_value(task, "project_id")
        if not project or "stage_id" not in task._fields:
            return request.env["project.task.type"]

        Stage = request.env["project.task.type"].sudo()
        status = (status or "").strip().lower()
        xml_ids = {
            "done": "industry_fsm.planning_project_stage_3",
            "cancelled": "industry_fsm.planning_project_stage_4",
            "canceled": "industry_fsm.planning_project_stage_4",
        }
        xml_id = xml_ids.get(status)
        if xml_id:
            stage = request.env.ref(xml_id, raise_if_not_found=False)
            if stage and project in stage.project_ids:
                return stage

        name_patterns = {
            "done": ["done", "completed", "complete", "منجز", "مكتمل", "منتهي"],
            "cancelled": ["cancelled", "canceled", "ملغي", "ملغاة"],
            "canceled": ["cancelled", "canceled", "ملغي", "ملغاة"],
        }.get(status, [])
        base_domain = [("project_ids", "in", project.id)]
        for pattern in name_patterns:
            stage = Stage.search(
                base_domain + [("name", "ilike", pattern)],
                order="sequence, id",
                limit=1,
            )
            if stage:
